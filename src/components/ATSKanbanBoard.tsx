...
import { Star, Sparkles, Search, Filter, CheckCircle2, UserCheck, Award, Briefcase, FileSpreadsheet, Calendar, LayoutGrid, Table as TableIcon, Eye } from 'lucide-react';
...
export const ATSKanbanBoard: React.FC<ATSKanbanBoardProps> = ({
  ...
  const isWithinTimeRange = (dateStr?: string) => {
    ...
    const diffDays = (now - targetDate) / (1000 * 3600 * 24);

    if (timeRange === 'week') return diffDays >= -1 && diffDays <= 7;
    if (timeRange === 'month') return diffDays >= -1 && diffDays <= 30;
    if (timeRange === 'year') return diffDays >= -1 && diffDays <= 365;
    return true;
  };

  // Filter candidates based on search, job filter, stage filter & time range
  const filteredCandidates = candidates.filter(c => {
    ...
    const matchesTime = isWithinTimeRange(c.appliedDate || c.lastUpdated);

    return matchesSearch && matchesJob && matchesStage && matchesTime;
  });
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-6">
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            <div className="flex items-center space-x-2 text-xs font-semibold text-indigo-400 mb-1">
              ...
              <span>Real-Time Candidate Pipeline & Google Sheets Sync</span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight font-display">
              Recruitment ATS Pipeline
            </h1>
            <p className="text-sm text-slate-400 mt-1">
              Track candidates across Round 1, Round 2, Round 3, Offer Letters & Onboarding with instant date filters.
            </p>
          </div>
        ...
        <div className="pt-4 border-t border-slate-800 flex flex-col lg:flex-row items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center justify-end gap-3 w-full lg:w-auto">
            {/* Time Filter Selector: Week / Month / Year */}
            <div className="flex items-center space-x-1 bg-slate-950 p-1 rounded-xl border border-slate-750 text-xs">
              <Calendar className="w-3.5 h-3.5 text-indigo-400 ml-2 mr-1" />
              <button
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-lg text-xs font-semibold transition-all ${
                  timeRange === 'year' ? 'bg-indigo-600 text-white shadow' : 'text-slate-400 hover:text-white'
                }`}
              >
                This Year
              </button>
                    ...
                    stageCandidates.map(candidate => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* Details grid */}
                          <div className="grid grid-cols-2 gap-1 text-[11px] text-slate-400 bg-slate-900/60 p-2.5 rounded-xl border border-slate-800/80">
                            <div>Exp: <strong className="text-slate-200">{candidate.experienceYears}</strong></div>
                            <div>Notice: <strong className="text-slate-200">{candidate.noticePeriod}</strong></div>
                            <div className="col-span-2 text-[10px] text-slate-400 truncate">
                              Salary: <strong className="text-slate-200">{candidate.expectedSalary}</strong>
                            </div>
                          </div>
                    ...
                    return (
                      ...
                      >
                        <td className="px-5 py-4">
                          <div>
                            <span className="text-[10px] font-mono text-indigo-400 block">{candidate.id}</span>
                            <span className="font-bold text-white group-hover:text-indigo-300 text-sm transition-colors font-display">
                              {candidate.fullName}
                            </span>
                          </div>
                        </td>
